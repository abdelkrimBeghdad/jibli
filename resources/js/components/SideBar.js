                           ...
                           <div key={item.id}>
                               <div className="row justify-content-xs-center">
                                   <div className="col ">
                                    <span >{item.name}</span>
                                   <br/>
                                  <span>{item.price} X {item.count} pc(s) </span> 
                                   </div>
                                   <div className="col">
                                       <div className="input-group mb-3">
                                       <div className="input-group-prepend" id="button-addon3">
                                           <button onClick={()=>this.props.decriseToCart(this.props.cartItems, item)} className="btn btn-outline-success rad" type="button">-</button>
                                           {/* <input type="" id="number" value={item.count} className='form-control form-control-xs' /> */}
                                           
   
                                           <div className="center">{item.count} </div>
                                           <button onClick={()=>this.props.addToCart(this.props.cartItems, item)} className="btn btn-outline-success rad" >+</button>
                                       </div>
                                         </div>
                                   </div>
                                   <div className="col ">
...
export default connect(mapStateToProps,{removeFromCart,addToCart,decriseToCart})(SideBar);